...
import { setSyncMode, reset as resetWorldSync } from "./DicomViewer/utils/WorldSyncStore";
...
const {
  ...
  // ── Stack DICOM 3-pane (mpr3): same crosshair hook, different viewport IDs ──
  useVolumeMprCrosshair({
    enabled: layoutMode === "mpr3" && !isNifti && crosshairEnabled && isCornerstoneDicom,
    mode: crosshairMode,
    renderingEngineRef,
    refs: [axRef, sagRef, corRef],
    vpIds: ["DICOM_SLOT_0", "DICOM_SLOT_1", "DICOM_SLOT_2"],
    readyToken: mprReadyToken,
  });
    ...
    /* ─── Linked 3D Pointer (IMAIOS / RadiAnt-style point cross-reference) ──
       Click (or drag) anywhere in a sync-grid pane → the other panes reslice to
       that patient-space point and draw their pointer there — exactly like
       RadiAnt / OsiriX / Horos / 3D Slicer. The "3D Pointer" toolbar button
       (crosshairEnabled) enables it for the compare/MPR grids. Active only with
       no primary-button tool selected (dicomTool === "none"); picking W/L, Pan,
       Zoom, or Measure hands the left button back to that tool. Not used in
       volMpr (the volume CrosshairsTool owns that). */
    useLinkedCrosshair({
      enabled:
        crosshairEnabled &&
        isCornerstoneDicom &&
      ...
      seriesUidForSlot: (slot) =>
        (layoutMode === "mpr3" ? mprViewportSeries : compareViewportSeries)?.[slot] || null,
      activeSlot: activeDicomSlot,
      setActiveSlot: setActiveDicomSlot,
      onAfterSync: refreshDicomSliceIndicators,
      crosshairMode,
    });
  ...
  // Auto-enter fullscreen on the FIRST user click after the viewer loads.
  // Browsers forbid going fullscreen on load (no user gesture), so we wait for
  // the first click, request fullscreen inside that gesture, then stop. We only
  // do it once, so if the user later presses Esc to leave, their next click
  // won't yank them back — they can re-enter with the fullscreen button.
  useEffect(() => {
    const enterOnce = () => {
      const el = viewerRootRef.current;
